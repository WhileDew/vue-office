...
import {renderImage, clearCache, buildImageMap} from './media';
...
export default defineComponent({
  ...
  setup(props, {emit}) {
    ...
    function renderExcel(buffer) {
      ...
      readExcelData(buffer, props.options.xls).then(async workbook => {
        ...
        let {
          ...
          medias,
          workbookSource
        } = transferExcelToSpreadSheet(workbook, {...defaultOptions, ...props.options});
    ...
    watch(() => props.src, async () => {
      if (props.src) {
        await getData(props.src, props.requestOptions).then(async (blob) => {
          await buildImageMap(blob)
          renderExcel(blob)
        }).catch(e => {
          mediasSource = [];
          workbookDataSource = {
